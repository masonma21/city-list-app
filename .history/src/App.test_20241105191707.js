...
test('sorts cities by name or state in both ascending and descending order', async () => {
  ...
  // The initial sort order is asscending (San José at the top)
  const initialCities = screen.getAllByText(/Chicago|El Paso|San José/i);
  expect(initialCities[0]).toHaveTextContent("San José");
  expect(initialCities[1]).toHaveTextContent("El Paso");
  expect(initialCities[2]).toHaveTextContent("Chicago");

  // Click the sort button to switch to ascending order
  const sortButton = screen.getByText(/Sort Ascending/i);
  ...
  // Click the sort button again to toggle back to descending order
  fireEvent.click(sortButton);

  // Check for descending order: "San José" should be first, followed by "El Paso" and "Chicago"
